...
**Check: stop-hook-v1:**
If `.claude/settings.json` has no `Stop` entry AND verify-all has 2+ meaningful checks AND CLAUDE.md type-check is set to a real command (not an opt-out value such as `none`, `none yet`, `n/a`, `skip`) AND not declined:
